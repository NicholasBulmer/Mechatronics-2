...
void irobot_rotate_continuous(INT16 rotateAngle, INT16 rotateSpeed){
	INT16 _rotateDirection;
	
	(rotateAngle > 0) ? (_rotateDirection = iR_CONST_CW) : (_rotateDirection = iR_CONST_CCW);
	irobot_move(0, rotateSpeed, _rotateDirection);
}
...
void update_bump_and_cliff(void){													//Updates bump and cliff sensors
	irobot_query_and_update(5, iR_PKT_BUMP_DROP, 
	iR_PKT_CLIFF_L, iR_PKT_CLIFF_FL, 
	iR_PKT_CLIFF_FR, iR_PKT_CLIFF_R);
}

//UPDATE ALL THE BUMP SENSORS, if
void update_bump(void){
    irobot_query_and_update(1, iR_PKT_BUMP_DROP);
}
...
void update_virtual_wall(void){
    irobot_query_and_update(1, iR_PKT_V_WALL);
}
