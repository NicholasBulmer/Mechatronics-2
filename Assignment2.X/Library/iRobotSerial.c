...
void rx_assign(UINT8 packetID, UINT8 packetIDValue, UINT8 byteNumber) { //Assign global values from the input data stream function
    switch (packetID) {
        case iR_PKT_BUMP_DROP: iRBumpDrop = packetIDValue;
            sort_iRBumpDrop();
            break;
        case iR_PKT_WALL: iRWall = packetIDValue;
            break;
        case iR_PKT_CLIFF_L: iRCliffL = packetIDValue;
            break;
        case iR_PKT_CLIFF_FL: iRCliffFL = packetIDValue;
            break;
        case iR_PKT_CLIFF_FR: iRCliffFR = packetIDValue;
            break;
        case iR_PKT_CLIFF_R: iRCliffR = packetIDValue;
            break;
        case iR_PKT_V_WALL: iRVirtualWall = packetIDValue;
            break;
        case iR_PKT_DISTANCE:
            (byteNumber == 0) ? (iRDistanceSS.HB = packetIDValue) : (iRDistanceSS.LB = packetIDValue,
                    iRDistance = MOVE_HIGH(iRDistanceSS.HB) + iRDistanceSS.LB);
            break;
        case iR_PKT_ANGLE:
            (byteNumber == 0) ? (iRAngleSS.HB = packetIDValue) : (iRAngleSS.LB = packetIDValue,
                    iRAngle = MOVE_HIGH(iRAngleSS.HB) + iRAngleSS.LB);
            break;
    }
}
...
void irobot_query_and_update(UINT8 numberOfPacketRequests, ...) {
    ...
    for (i = 0; i < streamSize; i++) {
        RXArray[i] = eusart_getch();
    } //Populate the RXArray
...
void irobot_script_play(void) { //Execute commands in iRScriptStack
    ...
    for (UINT8 i = 0; i < iRScriptIndex; i++) {
        eusart_putch(iRScriptStack[i]);
    } //Iterates through iRScriptStack and sends each element through EUSART
...
void irobot_rotate(bool isScript, INT16 rotateAngle, INT16 rotateSpeed) { //Rotate on the spot function
    ...
    (rotateAngle > 0) ? (rotateDirection = iR_CONST_CCW) : (rotateAngle = iR_CONST_CW);
    if (isScript) {
